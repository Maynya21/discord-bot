...
- `/play`, `/skip`, `/playlist`, `/stop` - 음성 채널 음악 재생
...
터미널을 열지 않고 더블클릭으로 실행할 수 있습니다.

| 파일 | 하는 일 |
|---|---|
| `start.bat` | 지금 받아둔 코드로 봇 실행 |
| `update.bat` | 최신 코드를 받고 의존성을 맞춘 뒤 실행 |

바탕화면에 두려면 파일을 **우클릭 > 바로 가기 만들기**로 만든 뒤 그 바로 가기를
옮기세요. 파일 자체를 옮기면 저장소 밖으로 나가서 동작하지 않습니다.

창은 봇이 멈춘 뒤에도 남습니다. 오류가 났을 때 읽을 수 있어야 하기 때문입니다.
끄려면 창에서 `Ctrl+C`.

배치 파일 안의 안내문은 영어입니다. `chcp 65001` 아래에서 cmd가 배치 파일 자체의
한글을 잘못 읽는 일이 있어서고, 봇이 찍는 한글 로그는 정상적으로 보입니다.
...
이름은 문장 어디에 있든 부른 것으로 봅니다. 메시지는 **손대지 않고 그대로**
넘어갑니다. 이름만 떼어내면 `차라는 어때?`가 `는 어때?`가 되어 뜻이 무너지는데,
발화를 `이름: 내용` 형태로 넘기고 있어 그대로도 자연스러운 채팅 로그로 읽힙니다.
...
- `/play <검색어 | 유튜브 링크 | 스포티파이 링크>` - 재생목록에 추가
- `/skip` - 지금 곡 건너뛰기
- `/playlist [remove]` - 목록 보기, 번호를 주면 그 곡을 뺌
- `/stop` - 멈추고 음성 채널에서 나가기

부른 사람이 있는 음성 채널로 따라 들어갑니다. 새 곡 없이 5분이 지나면 나갑니다.

### 재생목록과 반복

`/play`로 넣은 곡은 **재생목록에 쌓입니다.** 곡을 소모하지 않고 위치만 옮기기
때문에, 끝까지 가면 처음으로 돌아가 계속 돕니다. 패널의 🔁로 끄면 목록 끝에서
멈추고 새 곡을 기다립니다.

`/playlist`는 번호 붙은 목록을 보여주고, 지금 곡에 `▶` 표시가 붙습니다.
`/playlist remove:3`처럼 번호를 주면 그 곡이 빠집니다. 재생 중인 곡을 빼면
바로 다음 곡으로 넘어갑니다.
...
```
FFmpeg에서 -20dB 감쇠  →  퍼센트 조절(0~200%)  →  디스코드
   절대 크기                 그 위에서 상대 조절
```

**절대 크기는 `MUSIC_GAIN_DB`가 정합니다.** 퍼센트는 디스코드로 넘기기 직전에
곱하는 값이라, 들어오는 소리가 이미 크면 아무리 낮춰도 충분히 조용해지지
않습니다. 그래서 FFmpeg 단계에서 먼저 깎습니다.

```
MUSIC_GAIN_DB=-20     # 기본값. 여전히 크면 -26, -30으로 더 내리세요
```

패널의 **100%** 는 "원본 크기"가 아니라 "이 봇의 보통 크기"입니다. 🔊로
200%까지 올려도 원본보다는 조용합니다.
...
start.bat           # 더블클릭 실행 (윈도우)
update.bat          # 최신 코드를 받고 실행 (윈도우)
personas/
