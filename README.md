...
모든 응답은 임베드로 나갑니다. `Persona.color`가 임베드 왼쪽 색 띠로 표시되는데,
디스코드에서 기기를 가리지 않고 색을 넣을 수 있는 자리가 여기뿐이기 때문입니다.
ESC 문자도, 코드블록도 필요 없고 모바일에서도 그대로 보입니다. 색을 바꾸려면
`personas/<key>.py`의 `color` 값을 고치면 됩니다.
